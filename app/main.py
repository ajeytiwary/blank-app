...
@app.get('/api/health')
def health(): return {'ok':True,'version':'0.4.0','google':google_status()}
@app.get('/api/people')
def people(): c=db(); x=[d(r) for r in c.execute('SELECT * FROM people ORDER BY name COLLATE NOCASE').fetchall()]; c.close(); return x
@app.post('/api/people')
...
def add_interaction(i:InteractionIn): c=db(); cur=c.execute('INSERT INTO interactions(person_id,channel,summary,happened_at) VALUES(?,?,?,?)',(i.person_id,i.channel,i.summary,i.happened_at or now())); c.commit(); c.close(); return {'id':cur.lastrowid,'ok':True}
@app.post('/api/reminders')
def add_reminder(r:ReminderIn): c=db(); cur=c.execute('INSERT INTO reminders(person_id,text,due_at,created_at) VALUES(?,?,?,?)',(r.person_id,r.text,r.due_at,now())); c.commit(); c.close(); return {'id':cur.lastrowid,'ok':True}
@app.post('/api/ingest')
def ingestion(x:IngestIn): c=db(); out=ingest(c,x.text,x.source,x.person_id); c.close(); return out
@app.post('/api/voice-note')
def voice(x:IngestIn): return ingestion(IngestIn(text=x.text,source='voice',person_id=x.person_id))
@app.get('/api/inbox')
def inbox(): c=db(); x=[d(r) for r in c.execute('SELECT cf.*,p.name person_name FROM candidate_facts cf JOIN people p ON p.id=cf.person_id WHERE cf.status="pending" ORDER BY cf.created_at DESC').fetchall()]; c.close(); return x
...
def life_events(): c=db(); x=[d(r) for r in c.execute('SELECT le.*,p.name person_name FROM life_events le JOIN people p ON p.id=le.person_id ORDER BY le.created_at DESC').fetchall()]; c.close(); return x
...
@app.post('/api/demo/reset-and-seed')
def demo_seed():
 c=db()
 for table in ['life_events','candidate_facts','facts','reminders','interactions','source_entities','people']: c.execute(f'DELETE FROM {table}')
 c.commit(); out=seed_and_run(c); c.close(); return out
@app.get('/api/today')
def today():
